...
test("자신의 marker를 가진 봇 댓글만 갱신 대상으로 선택한다", () => {
  const comments = [
    { id: 1, user: { login: "octocat" }, body: "<!-- ai-review:v1 -->" },
    { id: 2, user: { login: "other-app[bot]" }, body: "<!-- ai-review:v1 -->" },
    { id: 3, user: { login: "github-actions[bot]" }, body: "일반 봇 댓글" },
    { id: 4, user: { login: "github-actions[bot]" }, body: "<!-- ai-review:v1 -->\n리뷰" },
  ];

  assert.equal(findExistingBotComment(comments)?.id, 4);
});
