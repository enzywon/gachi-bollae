...
같은 와이파이에 연결한 휴대폰으로 열려면 개발 서버를 모든 주소에서 받도록 띄우고, PC의 사설 IP를 `.env.local`에 등록합니다.

```bash
echo 'ALLOWED_DEV_ORIGINS=192.168.0.42' >> .env.local   # PC의 IP로 바꿔 주세요
npx next dev -H 0.0.0.0
```

Next.js는 등록하지 않은 주소에서 온 `/_next` 요청을 403으로 막습니다. 이때 HTML만 내려가고 스크립트가 전부 차단되어 **화면은 정상으로 보이는데 아무 버튼도 눌리지 않는** 상태가 되므로, 접속은 되는데 동작만 안 한다면 이 값을 먼저 확인하세요. 사설 IP는 공유기가 다시 배정하면 바뀔 수 있습니다.
